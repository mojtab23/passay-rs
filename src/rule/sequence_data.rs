...
impl EnglishSequenceData {
    /// Creates a new english sequence data.
    ///
    /// # Returns
    /// A tuple containing the error code and character sequences.
    pub fn new(&self) -> (String, Vec<CharacterSequence>) {
        match self {
            EnglishSequenceData::Alphabetical => (
                "ILLEGAL_ALPHABETICAL_SEQUENCE".to_string(),
            ...
            EnglishSequenceData::Numerical => (
                "ILLEGAL_NUMERICAL_SEQUENCE".to_string(),
                vec![
                    CharacterSequence::new(vec!["0123456789".to_string(), "".to_string()]).unwrap(),
                ],
            ),
            EnglishSequenceData::USQwerty => (
                "ILLEGAL_QWERTY_SEQUENCE".to_string(),
                vec![
